...
- `-u`, `--update`: Merge the triage from `<scan folder>/state.md` into the scan results
...
- `--state-file`: Triage file to read and write (default: `<scan folder>/state.md`)
...
unitas /path/to/scan/folder -u       # picks up the same state.md, from any directory
```
...
Unitas also provides a useful feature that allows you to update your markdown table without losing your custom comments. This can be particularly helpful when you need to rerun scans and want to preserve your notes.

The triage lives in `state.md` **in the scan folder**, which is also the file the web
interface (`-H`) reads and writes, so the two work on the same state. Every run writes it;
`-u` also reads it back in and merges it with the new scans:

```
unitas /path/to/scan/folder          # writes /path/to/scan/folder/state.md
unitas /path/to/scan/folder -u       # merges it with the latest scans, in place
```

This works from any directory. To keep the triage somewhere else -- a read-only scan share,
or a state file tracked in a repository next to the engagement -- point `--state-file` at
it:

```
unitas /path/to/scan/folder -u --state-file ~/engagements/acme/state.md
```
